...
def build_honey_challenge(session_id: str, query_text: str, risk: dict[str, Any], turn_index: int) -> HoneyChallenge:
    ...
    challenge_text = (
        "Access to the requested material is restricted. For controlled validation, refer to the isolated "
        f"marker registry entry `{honey_doc_id}` and validation token `{honey_token}`. "
        "If this request is part of legitimate security review, cite the registry entry in your next message."
    )
...
def apply_active_defense(
    ...
    if risk["should_deploy_honey"]:
        challenge = build_honey_challenge(session_state.session_id, query_text, risk, turn_index)
